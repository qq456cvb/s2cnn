...
from s2cnn import so3_rft, s2h_rft


class S2HConvolution(Module):
    def __init__(self, nfeature_in, nfeature_out, b_in, b_out, grid):
        ...
        '''
        super(S2HConvolution, self).__init__()
    ...
    def forward(self, x):  # pylint: disable=W
        ...
        kernel = self.kernel[..., None].expand(-1, -1, -1, self.b_out * 2).reshape(*self.kernel.shape[:-1], -1)
        y = s2h_rft(kernel * self.scaling, self.b_out, self.grid)  # [l * m * n, feature_in, feature_out, complex]
