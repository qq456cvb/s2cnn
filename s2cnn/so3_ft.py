...
def s2h_rft(x, b, grid):
    ...
    F = _setup_s2h_ft(b, grid, device_type=x.device.type, device_index=x.device.index)  # [beta_alpha_gamma, l * m * n, complex]

    # print(x.size(-1), F.size(0))
...
@cached_dirpklgz(hydra.utils.to_absolute_path("cache/setup_s2h_ft"))
def __setup_s2h_ft(b, grid):
    ...
    for i, (beta, alpha) in enumerate(grid):
        for j, gamma in enumerate(np.linspace(start=-max_gamma, stop=max_gamma, num=n_gamma, endpoint=False)):
            Dmats = [wigner_D_matrix(l, alpha, beta, gamma,
                                    field='complex', normalization='quantum', order='centered', condon_shortley='cs')
                    .conj()
                    for l in range(b)]
            F[i * n_gamma + j] = np.hstack([Dl.flatten() for Dl in Dmats])
...
@cached_dirpklgz(hydra.utils.to_absolute_path("cache/setup_so3_ft"))
...
@lru_cache(maxsize=32)
def _setup_s2h_ft(b, grid, device_type, device_index):
    F = __setup_s2h_ft(b, grid)
